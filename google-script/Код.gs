...
/** Сколько сделок пришло и закрылось именно сегодня. */
function заСегодня_(сделки) {
  var н = new Date(); н.setHours(0, 0, 0, 0);
  var начало = Math.floor(н.getTime() / 1000), конец = начало + 86400;
  var заявки = 0, договоры = 0, сумма = 0;
  сделки.forEach(function (с) {
    if (с.created_at >= начало && с.created_at < конец) заявки++;
    if (с.status_id === ВЫИГРАНА && с.closed_at >= начало && с.closed_at < конец) {
      договоры++; сумма += с.price || 0;
    }
  });
...
function заполнитьОтчёт() {
  ...
  var сделки = сделкиЗаМесяц_();
  var сегодня = заСегодня_(сделки.filter(function (с) { return ВОРОНКИ[с.pipeline_id]; }));
